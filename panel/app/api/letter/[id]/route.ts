...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.status && !(["Approved", "Confirmed", "Pending", "Flagged", "Rejected"].includes(body.status))) return NextResponse.json({ error: "status", message: "Your status field isn't Approved, Confirmed, Pending, Flagged, or Rejected!" }, { status: 400 });
  if (body.fulfilled !== undefined && body.fulfilled !== true && body.fulfilled !== false) return NextResponse.json({ error: "fulfilled", message: "Your fulfilled field isn't a boolean!" }, { status: 400 });
  if (!body.status && body.fulfilled === undefined) return NextResponse.json({ success: true, message: "Nothing to update!" });
